...
Dir.chdir(LIBGIT2_DIR) do
  ...
  Dir.chdir("build") do
    sys("cmake .. #{'-G "MSYS Makefiles" ' if RUBY_PLATFORM =~ /mswin(?!ce)|mingw|cygwin|bccwin/}-DBUILD_CLAR=OFF -DTHREADSAFE=ON -DBUILD_SHARED_LIBS=OFF -DCMAKE_C_FLAGS=-fPIC -DCMAKE_BUILD_TYPE=RelWithDebInfo")
    sys(MAKE)
